...
export default class CodeContextBuilder {
  ...
  buildCodeContext(editor, argType='Selection Based') {
    ...
    let codeContext = this.initCodeContext(editor);
  ...
  initCodeContext(editor) {
    ...
    let ignoreSelection = atom.config.get('script.ignoreSelection');
    ...
    let codeContext = new CodeContext(filename, filepath, textSource);
    codeContext.selection = selection;
    codeContext.shebang = this.getShebang(editor);

    let lang = this.getLang(editor);
