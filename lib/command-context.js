...
export default class CommandContext {
  ...
  static build(runtime, runOptions, codeContext) {
    let commandContext = new CommandContext;
  ...
  getRepresentation() {
    ...
    // command arguments
    let commandArgs = (this.options.cmdArgs != null) ? this.quoteArguments(this.options.cmdArgs).join(' ') : '';

    // script arguments
    let args = this.args.length ? this.quoteArguments(this.args).join(' ') : '';
    let scriptArgs = (this.options.scriptArgs != null) ? this.quoteArguments(this.options.scriptArgs).join(' ') : '';
