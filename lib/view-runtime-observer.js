...
export default class ViewRuntimeObserver {
  ...
  observe(runtime) {
    ...
    this.subscriptions.add(runtime.onDidExit(ev => this.view.setHeaderAndShowExecutionTime(ev.returnCode, ev.executionTime),
    ),
    );
    ...
    this.subscriptions.add(runtime.onDidContextCreate((ev) => {
      const title = `${ev.lang} - ${ev.filename}${(ev.lineNumber != null) ? `:${ev.lineNumber}` : ''}`;
      return this.view.setHeaderTitle(title);
    },
    ),
    );
    this.subscriptions.add(runtime.onDidNotSpecifyLanguage(() => this.view.showNoLanguageSpecified(),
    ),
    );
    this.subscriptions.add(runtime.onDidNotSupportLanguage(ev => this.view.showLanguageNotSupported(ev.lang),
    ),
    );
    this.subscriptions.add(runtime.onDidNotSupportMode(ev => this.view.createGitHubIssueLink(ev.argType, ev.lang),
    ),
    );
    return this.subscriptions.add(runtime.onDidNotBuildArgs(ev => this.view.handleError(ev.error),
    ),
    );
  }
