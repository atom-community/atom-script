...
export default class CodeContext {
  ...
  getCode(prependNewlines = true) {
    let code = __guard__(this.textSource, x => x.getText());
    if (!prependNewlines || !this.lineNumber) { return code; }

    let newlineCount = Number(this.lineNumber);
    let newlines = Array(newlineCount).join("\n");
  ...
  shebangCommand() {
    let sections = this.shebangSections();
    if (!sections) { return; }
