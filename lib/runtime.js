...
export default class Runtime {
  ...
  execute(argType = "Selection Based", input = null, options = null) {
    ...
    let codeContext = this.codeContextBuilder.buildCodeContext(atom.workspace.getActiveTextEditor(), argType);

    // In the future we could handle a runner without the language being part
    // of the grammar map, using the options runner
    if (__guard__(codeContext, x => x.lang) == null) { return; }

    let executionOptions = options ? options : this.scriptOptions;
    let commandContext = CommandContext.build(this, executionOptions, codeContext);
  ...
  modeNotSupported(argType, lang) {
    return this.emitter.emit('did-not-support-mode', { argType, lang });
  }

  didNotBuildArgs(error) {
    return this.emitter.emit('did-not-build-args', { error });
  }
};
Runtime.initClass();
