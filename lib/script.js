...
export default {
  ...
  activate(state) {
    ...
    if (state.profiles) {
      for (let profile of state.profiles) {
        let so = ScriptOptions.createFromOptions(profile.name, profile);
    ...
    let codeContextBuilder = new CodeContextBuilder;
    let runner = new Runner(this.scriptOptions);

    let observer = new ViewRuntimeObserver(this.scriptView);
    ...
    this.scriptOptionsView.onProfileSave(profileData => {
      ...
      let profile = ScriptOptions.createFromOptions(profileData.name, profileData.options);

      let codeContext = this.runtime.codeContextBuilder.buildCodeContext(atom.workspace.getActiveTextEditor(),
        "Selection Based");
      profile.lang = codeContext.lang;

      // formatting description
      let opts = profile.toObject();
      ...
      return this.scriptProfileRunView.setProfiles(this.scriptProfiles);
    }
    );

    // profile deleted
    this.scriptProfileRunView.onProfileDelete(profile => {
      let index = this.scriptProfiles.indexOf(profile);
      if (index === -1) { return; }
    ...
    this.scriptProfileRunView.onProfileChange(data => {
      let index = this.scriptProfiles.indexOf(data.profile);
      if (index === -1 || (this.scriptProfiles[index][data.key] == null)) { return; }
    ...
    return this.scriptProfileRunView.onProfileRun(profile => {
      if (!profile) { return; }
      return this.runtime.execute('Selection Based', null, profile);
    }
    );
  },
  ...
  // see https://github.com/s1mplex/Atom-Script-Runtime-Consumer-Sample for a full example
  provideBlankRuntime() {
    let runner = new Runner(new ScriptOptions);
    let codeContextBuilder = new CodeContextBuilder;
  ...
  serialize() {
    ...
    for (let profile of this.scriptProfiles) { serializedProfiles.push(profile.toObject()); }
