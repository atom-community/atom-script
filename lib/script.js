...
export function activate(state) {
  ...
  scriptProfileRunView.onProfileDelete((profile) => {
    ...
    if (index !== -1) { scriptProfiles.splice(index, 1); }
    scriptProfileRunView.setProfiles(scriptProfiles);
  });
  ...
  scriptProfileRunView.onProfileChange((data) => {
    ...
    if (index === -1 || !scriptProfiles[index][data.key]) { return; }
...
export function serialize() {
  ...
  for (const profile of scriptProfiles) { serializedProfiles.push(profile.toObject()); }
