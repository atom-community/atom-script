...
import { $$, View, SelectListView } from 'atom-space-pen-views';
...
export default class ScriptProfileRunView extends SelectListView {
  ...
  initializeView() {
    ...
    this.buttons = $$(function() {
      return this.div({class: 'block buttons'}, () => {
        ...
        this.button({class: "btn cancel"}, () => {
          return this.span({class: 'icon icon-x'}, 'Cancel');
        }
        );
        this.button({class: "btn rename"}, () => {
          return this.span({class: 'icon icon-pencil'}, 'Rename');
        }
        );
        this.button({class: "btn delete"}, () => {
          return this.span({class: 'icon icon-trashcan'}, 'Delete');
        }
        );
        return this.button({class: "btn run"}, () => {
    ...
    this.on('keydown', e => {
      ...
      if (e.keyCode === 13) { return this.run(); }
    }
    );
    ...
    let selector = '.rename, .delete, .run';
    if (this.profiles.length) { this.buttons.find(selector).show(); } else { this.buttons.find(selector).hide(); }

    this.panel = atom.workspace.addModalPanel({item: this});
    return this.panel.hide();
  }

  onProfileDelete(callback) { return this.emitter.on('on-profile-delete', callback); }
  onProfileChange(callback) { return this.emitter.on('on-profile-change', callback); }
  onProfileRun(callback) { return this.emitter.on('on-profile-run', callback); }

  rename() {
    let profile = this.getSelectedItem();
    if (!profile) { return; }

    let inputView = new ScriptInputView({caption: 'Enter new profile name:', default: profile.name});
    inputView.onCancel(() => this.show());
    inputView.onConfirm(newProfileName => {
      if (!newProfileName) { return; }
      return this.emitter.emit('on-profile-change', {profile, key: 'name', value: newProfileName});
    }
    );
  ...
  viewForItem(item) {
    return $$(function() { return this.li({class: 'two-lines profile'}, () => {
      this.div({class: 'primary-line name'}, () => {
        return this.text(item.name);
      }
      );
      return this.div({class: 'secondary-line description'}, () => {
